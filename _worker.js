...
export default {
  async fetch(request, env, ctx) {
    ...
    const sendTelegramPost = async (settings, postData, file = null) => {
      ...
      if (hash) captionLines.push(`Genre: ${hash}`);
      captionLines.push(`Release: ${esc(postData.release || '-')}`);
      captionLines.push(`➖➖➖➖➖➖➖➖➖➖`);
      captionLines.push(`📢 Channel: ${esc(channelLink)}`);
      captionLines.push(`👥 Group: ${esc(groupLink)}`);
      captionLines.push(`🌐 Website: ${esc(siteLink)}`);
